...
function TaskQuizObject({ view, assignmentTaskUUID, user_id }: TaskQuizObjectProps) {
 ...
 /* Checks whether the user is been graded or not */
  const { data: submissionData, isValidating, isLoading } = useSWR(
    `${getAPIUrl()}assignments/${assignment.assignment_object.assignment_uuid}/submissions/me`,
    (url: any) => swrFetcher(url, access_token)
  )
  const isGraded = submissionData?.[0]?.submission_status === "GRADED"
  ...
  if (questions && questions.length >= 0) {
    return (
      <AssignmentBoxUI submitFC={submitFC} saveFC={saveFC} gradeFC={gradeFC} view={view} currentPoints={userSubmissionObject?.grade} maxPoints={assignmentTaskOutsideProvider?.max_grade_value} showSavingDisclaimer={showSavingDisclaimer} user_id={user_id} type="quiz">
              ...
              <div className="flex flex-col space-y-2">
                {question.options.map((option, oIndex) => (
                  <div className="flex" key={oIndex}>
                    <div
                      {...(submissionData && submissionData[0]?.submission_status === "GRADED"
                    ...
                    >
                      ...
                      {view === 'student' && submissionData && submissionData[0]?.submission_status === "GRADED" ? (
                        <div
                        className={`w-[20px] flex-none flex items-center h-[20px] rounded-lg ${
                          (() => {
                            const isOptionSelectedByUser =
                            userSubmissionObject && 
                              userSubmissionObject.task_submission.submissions.find(
                                ({ questionUUID, optionUUID, answer }: any) =>
                                  questionUUID === question.questionUUID &&
                                  optionUUID === option.optionUUID &&
                                  answer
                              );
                    
                            if (option.assigned_right_answer && isOptionSelectedByUser) {
                              return "bg-green-200/60 text-green-500 hover:bg-green-300";
                            } else if (option.assigned_right_answer && !isOptionSelectedByUser) {
                              return "bg-blue-200/60 text-blue-500 hover:bg-blue-300";
                            } else if (!option.assigned_right_answer && isOptionSelectedByUser) {
                              return "bg-red-200/60 text-red-500 hover:bg-red-300";
                            }
                            return "";
                          })()
                        }`}
                      >
                          ...
                          {option.assigned_right_answer ? <Check size={12} className="mx-auto" /> : (userSubmissionObject && userSubmissionObject.task_submission.submissions.find(
                            (submission: any) =>
                        ...
                        (
                          <div
                            className={`w-[20px] flex-none flex items-center h-[20px] rounded-lg ${userSubmissions.submissions.find(
                              (submission) =>
                                submission.questionUUID === question.questionUUID &&
                                submission.optionUUID === option.optionUUID &&
                                submission.answer
